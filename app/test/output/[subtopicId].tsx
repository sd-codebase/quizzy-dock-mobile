...
export default function OutputTestScreen() {
  ...
  const loadQuestions = async () => {
    try {
      ...
      const data = await fetchOutputQuestions(params.subtopicId);
      if (data.length === 0) {
      ...
      console.error('Error loading output questions:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.mainContainer}>
        ...
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
          <View style={styles.content}>
            ...
            {/* Expected Output and Explanation (shown after Show Answer) */}
            {showExplanation && (
              <>
                <ExpectedOutputCard output={currentQuestion.output} />
